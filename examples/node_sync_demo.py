...
def main():
    ...
    print("Before sync")
    print(f"Node A chain length: {len(node_a.engine.chain)}")
    print(f"Node B chain length: {len(node_b.engine.chain)}")

    adopted = node_b.sync_from_peer(node_a)
    ...
    print(f"Node A chain length: {len(node_a.engine.chain)}")
    print(f"Node B chain length: {len(node_b.engine.chain)}")
    print(f"Node B valid: {node_b.engine.is_chain_valid()}")
